...
2. 클릭 링크에 u_id를 붙여 클릭 이동[(3. u_id를 click과 연동하는 방법 참조)](#uid)
...
6. 리워드 전송
   ...
   * 링크프라이스로부터 리워드 데이터를 받아 처리할 페이지 개발 및 등록[(4. 리워드 승인 신청 참조)](#rewardJoin)
...
- 실적 발생 시 머천트에서 링크프라이스로 실적을 보내주면, 링크프라이스에서 어필리에이트로 리워드를 전송합니다. 링크프라이스가 전송한 데이터를 받아 처리할 페이지가 개발 되어야 하며, 그 주소를 리워드 승인 후 리워드 API 받을 주소에 저장하셔야 합니다.
