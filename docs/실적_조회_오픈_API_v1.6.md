...
본 문서는 어필리에이트의 리워드 서비스를 위해 링크프라이스에서 제공하는 실적 조회 API 사용 가이드입니다. 기간내 발생한 실적 정보를 조회하고 해당 실적의 상태를 확인할 수 있습니다. 어필리에이트 아이디와 인증키의 발급은 링크프라이스 어필리에이트 담당자에게 문의 부탁드립니다. 

문의 메일 : affiliate@linkprice.com
...
>
> api.linkprice.com/affiliate/translist.php ?a_id=[어필아이디] & auth_key=[인증키] & yyyymmdd=20231120
...
>
> api.linkprice.com/affiliate/translist.php ?a_id=[어필아이디] & auth_key=[인증키] & yyyymmdd=20231120 & cancel_flag=N
...
>
> api.linkprice.com/affiliate/translist.php ?a_id=[어필아이디] & auth_key=[인증키] & yyyymmdd=20231120 & cancel_flag=Y
...
| :-------------: | :------: | :------: | :----------: |---------------------------------------------------------------------------------------------------------------------------------------|
|      a\_id      | VARCHAR  |    10    |     필수     | 링크프라이스에 가입된 어필리에이트 아이디                                                                                                                |
...
|       hhmiss        | VARCHAR  |                   8                   | 주문 시간                                                    |
|   trans\_comment    | VARCHAR  |                 1000                  | 실적 취소 사유(값이 있을 경우에만)                           |
|        sales        |   INT    |                  11                   | 판매 합계금액                                                |
|     commission      |   INT    |                  11                   | 어필 커미션                                                  |
...
```json
GET https://api.linkprice.com/affiliate/translist.php?a_id=[어필아이디]&auth_key=[인증키]&yyyymmdd=20231120
{
    "result": "0",
    "list_count": 2,
    "order_list": [
...
- 조회일자(yyyymmdd) 누락
```json
GET https://api.linkprice.com/affiliate/translist.php?a_id=[어필아이디]&auth_key=[인증키]
{
    "result": "200",
    "list_count": 0,
    "order_list": []
