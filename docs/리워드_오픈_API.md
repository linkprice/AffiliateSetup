...
1. 배너 클릭
2. 클릭시 링크에 어필리에이트 회원구분을 위한 u_id를 함께 전송하여 이동[(3. u_id를 click과 연동하는 방법 참조)](#uid)
3. 어필리에이트 정보와 함께 머천트 상품페이지로 이동

4. 어필리에이트 회원의 상품 구매시 머천트는 실적 데이터를 링크프라이스로 전송
5. 링크프라이스는 실적 기록과 함께 어필리에이트에게 리워드 데이터 전송
...
6. 매달 1번 링크프라이스에서 머천트측으로 정산 요청
7. 실적 확정 및 취소
8. 머천트 정산 방식에 따라 일부 실적 리워드가 전송[(2. 리워드 주의사항 참조)](#warning)
9. 실적 조회 API를 통하여 링크프라이스측으로 실적 조회 요청[(실적조회 API)](https://github.com/linkprice/AffiliateSetup/blob/master/docs/%EC%8B%A4%EC%A0%81_%EC%A1%B0%ED%9A%8C_%EC%98%A4%ED%94%88_API_v1.6.md)
10. 링크프라이스는 요청받은 실적 데이터 정보 제공
11. 응답받은 확정 및 취소 데이터 확인 후 리워드 확정
