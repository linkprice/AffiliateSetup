...
6. <a link="#1-6"></a>링크프라이스에서 어필리에이트에게 발생한 실적의 리워드 데이터 전송
...
8. 실적 확정 및 취소
9. <a link="#1-9"></a>머천트 정산 방식에 따라 일부 실적 리워드 전송[(2. 리워드 주의사항 참조)](#warning)
...
11. <a link="#1-11"></a>링크프라이스는 요청받은 실적 데이터 정보를 제공
